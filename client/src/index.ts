...
const comboBoxElement = document.getElementById('comboBox');
...
function updateInfo() {
  ...
  comboBoxElement.style.fontSize = `${comboBoxSize}px`;
}
...
function shrinkComboBox(dt: number) {
  if(!comboBoxSize) return;
  comboBoxSize -= 150 * dt;
  if(comboBoxSize >= 0) return;
  comboBoxSize = 0; 
}
